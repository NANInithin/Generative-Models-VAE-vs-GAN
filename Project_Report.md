# TP1 Report: Generative Models (VAE vs GAN)
...
```
L_VAE = E[BCE(x, x_hat)] + KL(N(μ, σ²) || N(0, I))
```
...
```
W(P_r, P_g) = inf { E[||x - y||] : γ is a joint distribution of (P_r, P_g) }
            γ
```
...
```
W(P_r, P_g) ≈ max { E_x[D(x)] - E_z[D(G(z))] : D is 1-Lipschitz }
            D
```
...
```
For each weight w in Critic: w = clip(w, -0.01, 0.01)
```
...
Critic Loss Improvement = (0.1308 - 0.0171) / 0.1308 × 100 = 86.9%

Generator Loss Stability: Range = 0.0350 - 0.0057 = 0.0293 (essentially flat)
...
- Suitable for research on compact VLMs for edge computing
...
1. **Gradient Vanishing Problem:** The Sigmoid activation in the discriminator output leads to saturated gradients when the discriminator is confident:
   ```
   d/dD[log(1-D(G(z)))] → 0 as D(G(z)) → 0
   ```
...
1. **Meaningful Gradient Signal:** The Wasserstein distance provides non-zero gradients even when distributions are disjoint:
   ```
   W(P_r, P_g) = max { E[D(x)] - E[D(G(z))] : D is 1-Lipschitz }
   ```

2. **Lipschitz Constraint:** Weight clipping enforces the 1-Lipschitz constraint:
   ```
   ||D(x₁) - D(x₂)|| ≤ ||x₁ - x₂|| for all x₁, x₂
   ```
